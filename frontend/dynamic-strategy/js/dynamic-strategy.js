...
(function() {
    ...
    DS.prototype.init = (items) => {
        ...
        document.querySelector('inventory-items').addEventListener('itemclick', (evt) => {
            console.log("item click", evt.detail);
        ...
        document.querySelector('inventory-items').addEventListener('addtocart', (evt) => {
            ...
            let inventoryItem = evt.detail;
            if (inventoryItem instanceof Array) {
                inventoryItem = inventoryItem[0];
            }
            if (inventoryItem && cartStore.cartItems) {
                let itemInCart = cartStore.cartItems.find(item => item.code === inventoryItem.code);
                if (itemInCart) {
                    ...
                    let itemToAdd = {
                        ...
                        quantity: 1,
                        stockPrice: inventoryItem.stockPrice,
                        _raw: inventoryItem
                    };
                    api.post({
                        url: `http://localhost:5001/api/cart-items`,
